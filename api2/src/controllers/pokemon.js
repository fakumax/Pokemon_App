...
const getAllPokeAPI = async (req, res, next) => {
  const { name } = req.query;
  console.log('wara', req.query);
  if (name) {
    //----------------------------------------
    try {
    ...
    try {
      ...
      //count += 40;
      const filterURL = await url.data.results.map((value) => axios.get(`${value.url}`));
...
export { postPokemon, getAllPokeAPI, getPokemonId };
